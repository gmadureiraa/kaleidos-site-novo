...
import { blogNew69 } from "./blog-new-69";
import { blogSetGtmFintech } from "./blog-set-gtm-fintech";
import { blogMarcaPessoal } from "./blog-marca-pessoal";
...
function rankRelated(current: BlogPost, others: BlogPost[], limit: number): BlogPost[] {
  ...
  return others
    .map((p) => {
      ...
      // Categoria pesa MAIS que qualquer soma de tags (peso 100 vs. ~1 por tag).
      // Motivo: o blog é ~90% cripto — com peso 2 um post de marca pessoal caía
      // em "relacionados" de cripto por coincidência de tag genérica ("conteúdo",
      // "marketing"), e o leitor da trilha saía dela. Categoria = a trilha.
      const score = (p.category === current.category ? 100 : 0) + shared;
