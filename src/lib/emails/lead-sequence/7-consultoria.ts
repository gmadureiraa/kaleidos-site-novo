...
export function buildConsultoriaEmail(opts: { name?: string | null; unsubUrl: string }) {
  ...
  const body = `
    ...
    <p style="margin:0 0 14px 0;">
      ...
      é mais direto: <strong>uma consultoria gratuita de 30 minutos</strong> com o nosso
    ...
    ${ctaButton("Agendar minha consultoria gratuita", AGENDAR_URL)}
    ...
    <p style="margin:24px 0 0 0;font-size:13px;color:#9ca3af;border-top:1px solid #f1f1ef;padding-top:14px;">
      As vagas de consultoria são limitadas pela agenda do time, então se o assunto é
  ...
  return {
    subject,
    html: shell({
      preheader: "Consultoria gratuita de 30 minutos, focada 100% no teu projeto. Escolhe teu horário.",
      bodyHtml: body,
      unsubUrl: opts.unsubUrl,
    }),
    text: `Bora colocar o teu cenário na mesa?

Ao longo das últimas semanas eu te mandei perguntas, teses e playbooks. Hoje o convite é mais direto: uma consultoria gratuita de 30 minutos com o nosso time, focada 100% no teu projeto.
...
Agendar minha consultoria gratuita: ${AGENDAR_URL}
...
As vagas de consultoria são limitadas pela agenda do time, então se o assunto é urgente, garante o teu horário logo.
